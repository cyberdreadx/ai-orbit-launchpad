...
const JumpstartPackage = () => {
  const packages = [{
    ...
    features: ["1 basic automation (form to Google Sheet or email)", "Basic intake form (Tally)", "No ChatGPT logic", "Runs on free-tier Zapier", "1 revision included"],
    ...
    features: ["Intake form → spreadsheet + email summary", "AI summary using GPT", "Spreadsheet CRM tracking", "Zapier setup (up to 2 steps)", "Email automation setup", "1 onboarding call"],
    ...
    features: ["All from Starter +", "AI workflows with ChatGPT", "Multi-step Zapier automation", "Client CRM (Notion, Sheet, or Airtable)", "1 month monitoring + tweaks", "Help with setting up Zapier, Tally, etc.", "Content automation add-on (optional)"],
    ideal: "Great for agencies, consultants, or digital businesses"
  }];
  return <section id="packages" className="py-20 px-4 relative">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {packages.map((pkg, index) => <Card key={index} className={`bg-white/5 backdrop-blur-lg border-white/10 hover:bg-white/10 transition-all duration-300 hover:scale-105 relative ${pkg.popular ? 'ring-2 ring-purple-500/50' : ''}`}>
              ...
              <CardContent className="space-y-6">
                <ul className="space-y-3">
                  {pkg.features.map((feature, idx) => <li key={idx} className="flex items-start gap-3 text-white/90 text-sm">
                ...
                <div className="border-t border-white/10 pt-4">
                  ...
                  <Button size="lg" className={`w-full bg-gradient-to-r ${pkg.gradient} hover:opacity-90 text-white py-4 text-base rounded-full shadow-xl transition-all duration-300 hover:scale-105`} onClick={() => window.open('https://calendly.com/eharouge/30min', '_blank')}>
